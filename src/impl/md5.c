 ...
 ** Adaptation for the CKit library                                  **
 **********************************************************************
 */
...
{
  MD5_CTX *mdContext = (MD5_CTX *)ckit_alloc(sizeof(MD5_CTX));
...
void md5_finalize(MD5_CTX *mdContext, unsigned char digest[16])
{
  UINT4 in[16];
  int mdi;
  unsigned int i, ii;
...
UString *md5_string(const unsigned char *buf, unsigned int len){
    ...
    int pos = 0;
    for(int i = 0; i < 16; i++){
        outbuf[pos++] = HEXA[(digest[i] & 0xf0) >> 4];
